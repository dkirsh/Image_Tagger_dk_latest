function [ pixV, pixT] = Count_Colors_variable( Folder_Address, ctrs ,thRGB, thS) 
...
sz = get(0,'screensize');
figure('Position',[0 0 round(0.9*sz(3)) round(0.65*sz(4))],'color','w')

for s = 1:num_image
    imnamee = names(s).name;
    imname{s} = imnamee;
    I = imread(imnamee);
    [s1, s2, s3] = size(I);
    %thR = 0.15; 
    ...
    if s3 == 3
        ...
        pixV(s,:) = [pixR pixY pixG pixC pixB pixM pixW];
        pause(2)
        clf
    end
end
