...
They depend on LightGBM models trained on VLM-annotated data and
should be treated as provisional until validated against ground truth.
...
class AffordanceAnalyzer:
    ...
    WARNING: All outputs are L3 HYPOTHESES. The LightGBM models were
    trained on VLM-annotated data and should be treated as provisional.
    ...
    _MODEL_NAME = "lgbm_affordance"
    _PROMPT_HASH = "affordance_indicator_v1"
    ...
    def analyze(self, frame: AnalysisFrame) -> None:
        ...
        _SRC = "affordance.AffordanceAnalyzer.analyze"
        ...
        try:
            ...
            for aff_id in AFFORDANCE_IDS:
                ...
                frame.add_hypothesis(
                    f"affordance.{aff_id}", score,
                    source=_SRC, model_name=_MN, prompt_hash=_PH,
                    confidence=0.85,
                )
                frame.add_hypothesis(
                    f"affordance.{aff_id}_norm", normalized,
                    source=_SRC, model_name=_MN, prompt_hash=_PH,
                    confidence=0.85,
                )
