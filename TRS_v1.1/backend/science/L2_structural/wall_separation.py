...
class WallSeparationAnalyzer:
    ...
    name = "wall_separation"
    tier = "L2"
    requires = ["segmentation_masks"]
    provides = ["wall.count"]

    _MODEL_VERSION = "depth_corner_color_v1"
    ...
    def analyze(self, frame: "AnalysisFrame") -> None:
        ...
        _SRC = "wall_separation.WallSeparationAnalyzer.analyze"
        masks = getattr(frame, 'segmentation_masks', {})
        wall_mask = masks.get('wall')
        
        if wall_mask is None or not wall_mask.any():
            frame.wall_regions = []
            frame.add_structural("wall.count", 0, model_version=_MV, source=_SRC)
            return
        ...
        if depth_map is None:
            ...
            frame.add_structural("wall.count", 1, model_version=_MV, source=_SRC)
            return
        ...
        frame.add_structural("wall.count", len(regions), model_version=_MV, source=_SRC)
        
        # Add per-wall attributes
        for wall in regions:
            prefix = f"wall.{wall.wall_id}"
            frame.add_structural(f"{prefix}.area_fraction", wall.area_fraction, model_version=_MV, source=_SRC)
            frame.add_structural(f"{prefix}.mean_depth", wall.mean_depth, model_version=_MV, source=_SRC)
