...
class SegmentationAnalyzer:
    ...
    provides = ["segmentation_masks"]
    
    MODEL_ID = "nvidia/segformer-b5-finetuned-ade-640-640"
    _MODEL_VERSION = "segformer_b5_ade20k"
    ...
    def analyze(self, frame: "AnalysisFrame") -> None:
        ...
        _SRC = "segformer.SegmentationAnalyzer.analyze"
        ...
        self._add_coverage_attributes(frame, _MV, _SRC)
    ...
    def _add_coverage_attributes(self, frame: "AnalysisFrame", model_version: str, source: str) -> None:
        ...
        for region_name, mask in masks.items():
            ...
            frame.add_structural(f"region.{region_name}.coverage", coverage, model_version=model_version, source=source)
...
class MockSegmentationAnalyzer:
    ...
    _MODEL_VERSION = "mock_geometric_v1"
    
    def analyze(self, frame: "AnalysisFrame") -> None:
        """Generate mock segmentation based on image geometry."""
        _MV = self._MODEL_VERSION
        _SRC = "segformer.MockSegmentationAnalyzer.analyze"
        ...
        for region_name, mask in frame.segmentation_masks.items():
            coverage = float(mask.sum()) / total_pixels
            frame.add_structural(f"region.{region_name}.coverage", coverage, model_version=_MV, source=_SRC)
