...
Moved to L2_structural with add_structural() API.
"""
...
class LocalizedAnalyzer:
    ...
    name = "localized_pipeline"
    tier = "L2"
    requires = ["segmentation_masks"]
    provides = [
        "region.*.fractal_d",
        "contrast.ceiling_floor.fractal_d",
        "contrast.wall_ceiling.fractal_d",
        "contrast.focal_flanking.fractal_d",
    ]

    _MODEL_VERSION = "box_counting_fractal_v1"
    ...
    def analyze(self, frame: "AnalysisFrame") -> None:
        ...
        """
        _MV = self._MODEL_VERSION
        _SRC = "localized_pipeline.LocalizedAnalyzer.analyze"
        ...
        if frame.edges is not None:
            ...
            # 2. Pool to regions
            self._pool_fractal_to_regions(frame, dense_fractal, valid_mask, _MV, _SRC)
            
            # 3. Pool to wall regions if available
            self._pool_to_wall_regions(frame, dense_fractal, valid_mask, _MV, _SRC)
        
        # 4. Compute contrasts
        self._compute_contrasts(frame, _MV, _SRC)
    ...
    def _pool_to_wall_regions(
        ...
        for wall in wall_regions:
            ...
            if not np.isnan(stats.mean):
                ...
                frame.add_structural(f"{prefix}.fractal_d", stats.mean, model_version=model_version, source=source)
                frame.add_structural(f"{prefix}.fractal_d_std", stats.std, model_version=model_version, source=source)
    
    def _compute_contrasts(self, frame: "AnalysisFrame", model_version: str, source: str) -> None:
        ...
        if ceil_fd is not None and floor_fd is not None:
            ...
            frame.add_structural('contrast.ceiling_floor.fractal_d', diff, model_version=model_version, source=source)
        ...
        if wall_fd is not None and ceil_fd is not None:
            ...
            frame.add_structural('contrast.wall_ceiling.fractal_d', diff, model_version=model_version, source=source)
        ...
        if focal_fd is not None and flanking_fds:
            ...
            frame.add_structural('contrast.focal_flanking.fractal_d', diff, model_version=model_version, source=source)
