...
class MCubeS(Dataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Tensor, Tensor]:
        ...
        # Paths
        paths = {
            'image': os.path.join(self.root, 'polL_color', item_name+'.png'),
            'aolp_sin': os.path.join(self.root, 'polL_aolp_sin', item_name+'.npy'),
            'aolp_cos': os.path.join(self.root, 'polL_aolp_cos', item_name+'.npy'),
            'dolp': os.path.join(self.root, 'polL_dolp', item_name+'.npy'),
            'nir': os.path.join(self.root, 'NIR_warped', item_name+'.png'),
            'label': os.path.join(self.root, 'GT', item_name+'.png'),
            'nir_mask': os.path.join(self.root, 'NIR_warped_mask', item_name+'.png'),
            'mask': os.path.join(self.root, 'SS', item_name+'.png')
        }

        sample = {}

        # Load only requested modalities
        if 'image' in self.modals:
            _img = cv2.imread(paths['image'], -1)[:, :, ::-1]
            _img = _img.astype(np.float32)/65535 if _img.dtype == np.uint16 else _img.astype(np.float32)/255
            sample['image'] = _img

        if 'aolp' in self.modals:
            _aolp_sin = np.load(paths['aolp_sin'])
            _aolp_cos = np.load(paths['aolp_cos'])
            _aolp = np.stack([_aolp_sin, _aolp_cos, _aolp_sin], axis=2)
            sample['aolp'] = _aolp

        if 'dolp' in self.modals:
            dolp = (np.load(paths['dolp']) + 0.55) / 2.0
            _dolp = np.stack([dolp, dolp, dolp], axis=2)
            sample['dolp'] = _dolp

        if 'nir' in self.modals:
            _nir = cv2.imread(paths['nir'], -1)
            _nir = _nir.astype(np.float32)/65535 if _nir.dtype == np.uint16 else _nir.astype(np.float32)/255
            _nir = np.stack([_nir, _nir, _nir], axis=2)
            sample['nir'] = _nir

        # Labels and masks are usually always needed
        _target = cv2.imread(paths['label'], -1)
        _mask = cv2.imread(paths['mask'], -1)
        _nir_mask = cv2.imread(paths['nir_mask'], 0)

        sample['label'] = _target
        sample['mask'] = _mask
        sample['nir_mask'] = _nir_mask
        sample['u_map'] = self.u_map
        sample['v_map'] = self.v_map

        # Crop if left offset is set
        for k in sample:
            if sample[k] is not None:
                sample[k] = sample[k][:, self._left_offset:]
        ...
        # Return only requested modalities
        sample_out = [sample[k] for k in self.modals]

        return sample_out, label
