...
**Design substance (from the David↔Stephan Zoom, `/Users/davidusa/REPOS/_control/METHODOLOGY/ZOOM_STEPHAN_DK_IMAGE_SEARCH_2026-08-15.md` §3).** The loop is deliberately *not* an opaque image→3D conversion; the Zoom fixes commitments that shape every task below:
- **A semantic scene representation sits between the photo and the environment** (§3.1). Interpret the image first in language-like categories — entities and features (tables, floor-to-ceiling glazing, window types, furniture, materials, spatial relations) — which populate the scene graph that then guides asset retrieval and assembly. Do not go straight from pixels to a room volume; a bare volume was exactly the early mock-up's failure.
- **Classify coarse first** (§3.1). Recognize broad categories before branching across hundreds of specific meshes; later stages retrieve candidate assets approximating the detected objects. Branching immediately into a huge catalogue makes the decision space unmanageable.
- **Semantic ground truth ≠ geometric ground truth** (§3.2). Knowing a thing is a table fixes neither its shape, size, pose, nor coordinates. Datasets pairing photographs with 3D scans — **ScanNet++** — give stronger geometric reference than labels alone and can serve as a geometric ground-truth benchmark for reconstruction quality, camera alignment, and object placement.
- **Camera/viewpoint alignment IS the evaluation mechanism** (§3.3). Align the virtual camera to the source photograph's viewpoint, render from that pose, and compare render against original side by side. Without alignment a reviewer must mentally correct for viewpoint, which makes even basic correspondence hard to judge. This is the *render* step of the render↔verdict cycle, and it is what makes the comparison a legitimate test rather than a resemblance vibe.
- **The camera-model caveat** (§3.4). Focal length, field of view, lens distortion, sensor, and unknown camera position all affect the comparison; a human-vision-approximating VR camera will not reproduce an unknown lens. Recover or estimate the original camera metadata where the experiment's fidelity needs justify it — a lower-order concern early, but not an ignored one.
- **Success is experiment-adequacy, not photorealism** (§3.5). The target is an environment good enough for controlled student experiments with explicit tolerances and stable conditions; substituting one similar table for another is tolerable if the same asset and placement hold across experimental and control conditions. Pixel identity is not the bar.
- **Human-visible sanity checks** (§2.5). Load the actual generated model in Blender (or a viewer) rather than asking a model to render a lookalike image; a human catches gross geometry, coordinate, and correspondence failures fast. Expose intermediate artifacts.

The loop's **input** is a selected image handed over by Stephan's search lane through the **search→reconstruction handoff contract** (defined on his side: `New_VR_Platform/docs/STEPHAN_SPRINT_SET_2026-08-14.md` T1.6). That seam and the render↔verdict seam are the two interfaces this loop lives between.
