...
| Integration algorithm | Full BFS shortest-path (scipy) | Turner, Doxa, O'Sullivan & Penn 2001, "From isovists to visibility graphs", E&PB 28(1), 103–121 | Full graph-theoretic integration: BFS shortest path from each sampled node → mean visual depth → relative asymmetry → Turner integration (1/RA). Replaces earlier 2-step approximation (adversarial finding #3). Algorithm now matches space_syntax.vga_metrics() for consistency. | With max_nodes=2000, BFS is O(N²) — ~4M operations, still <2s. Subsampled grids interpolate via morphological dilation. |
